...
// This uses the default credentials from the environment or application default credentials
admin.initializeApp({
    projectId: 'nspc-web'
});
...
async function seedPageContent() {
    ...
    try {
        for (const [pageId, content] of Object.entries(pageContents)) {
            await db.collection(`${siteId}_content`).doc(pageId).set({
                ...content,
                siteId,
...
async function seedVideos() {
    ...
    try {
        for (const video of videos) {
            await db.collection(`${siteId}_videos`).add({
                ...video,
...
async function seedPartners() {
    ...
    try {
        for (const partner of partners) {
            await db.collection(`${siteId}_partners`).add({
                ...partner,
                createdAt: new Date().toISOString(),
                updatedAt: new Date().toISOString()
            });
