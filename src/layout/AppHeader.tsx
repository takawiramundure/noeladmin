      ...
      <div className="flex flex-col items-center justify-between grow lg:flex-row lg:px-6">
        ...
        >
          <div className="flex items-center gap-2 2xsm:gap-3">
            {/* Preview Mode Toggle */}
            <div className="flex items-center gap-1.5 px-2.5 py-1 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700" title="Preview Mode: controls what you see in the admin — saves always go live instantly">
              <span className={`text-[10px] font-extrabold ${isDraftMode ? 'text-amber-600 dark:text-amber-400' : 'text-gray-400'}`}>PREVIEW</span>
              <button
                type="button"
                onClick={toggleDraftMode}
                className={`w-7 h-4 rounded-full transition-colors relative ${isDraftMode ? 'bg-amber-500' : 'bg-green-600'}`}
                title="Toggle admin preview between your edits (amber) and the public view (green). Saves always go live instantly regardless of this toggle."
              >
              ...
              <span className={`text-[10px] font-extrabold ${!isDraftMode ? 'text-green-600 dark:text-green-400' : 'text-gray-400'}`}>PUBLIC</span>
            </div>

            {/* ⚡ Saves are always live — this button force-syncs draft → live for edge cases */}
            {getPageIdFromUrl() && (
              <button
                type="button"
                ...
                title="Saves automatically go live. Use this to force-sync if anything looks out of date."
              >
                {publishing ? "Syncing..." : "⚡ Sync Live"}
              </button>
            )}
